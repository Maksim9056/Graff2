...
namespace ConsoleApp2
{
    public class Graf
    {
        ...
        private void TopologicalSortUtil(string node, HashSet<string> visited , Stack<string> stack)
        {
            visited.Add(node);
            ...
            foreach (string nextNode in graph[node])
            {
                if (!visited.Contains(nextNode))
                {
                   ...
                   // Console.WriteLine($"Следующия нода {nextNode} поситили  { visited}   Стек {stack}");
